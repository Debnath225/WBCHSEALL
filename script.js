...
document
  ...
  .addEventListener("click", () => loadJSON(true));
document
  .getElementById("closeFinal")
  .addEventListener("click", () => (finalModal.style.display = "none"));
document
  .getElementById("exportJSON")
  .addEventListener("click", exportResultsJSON);
document
...
function renderQuestion() {
  ...
  if (q.type === "identify_formula") {
    ...
    qtext = `In the formula "${q.formula || ""}", what does "${
      q.term
    }" represent?`;
    hintContent.textContent = q.description || "No hint available";
  } else {
  ...
  const pct = Math.round(
    (currentIndex / Math.max(1, shuffledQuestions.length)) * 100
  );
  progBar.style.width = pct + "%";
  numQEl.textContent = shuffledQuestions.length;
  scoreEl.textContent = score;
  scorePercent.textContent = `${Math.round(
    (score / Math.max(1, shuffledQuestions.length)) * 100
  )}%`;
  scoreText.textContent = `Answered: ${currentIndex} • Remaining: ${Math.max(
    0,
    shuffledQuestions.length - currentIndex
  )}`;
  metaRight.textContent = `Loaded chapters: ${
    dataJSON && dataJSON.chapters ? dataJSON.chapters.length : 0
  }`;
}
...
function runConfetti() {
  if (!confettiRunning && confettiItems.length === 0) {
    clearCanvas();
    return;
  }
  ...
  if (confettiRunning || confettiItems.length)
...
function showFinal() {
  ...
  finalMsg.textContent =
    pct >= 80
      ...
      : pct >= 50
      ? "Good effort — revise the weak areas."
...
function exportResultsJSON() {
  const blob = new Blob(
    [
      JSON.stringify(
        { results, score, total: shuffledQuestions.length },
        null,
        2
      ),
    ],
...
function exportResultsCSV() {
  const rows = [
    ["index", "question", "selected", "correct", "isCorrect", "chapter"],
  ];
  results.forEach((r) =>
    rows.push([
      r.index,
...
function restartQuiz() {
  ...
  if (shuffledQuestions.length < 6)
    shuffledQuestions = prepareQuestions(
      questionPool,
      Math.max(6, shuffledQuestions.length)
    );
  currentIndex = 0;
  score = 0;
  results = [];
...
function prepareQuestions(pool, desired) {
  ...
  let i = 0;
  while (
    chosen.length < desired &&
    (byType.identify_formula.length || byType.term_meaning.length)
  ) {
    // alternate picking type when possible
    const pickType =
      i % 2 === 0 && byType.identify_formula.length
        ? "identify_formula"
        : byType.term_meaning.length
        ? "term_meaning"
        : "identify_formula";
    if (byType[pickType].length) {
      const item = byType[pickType].splice(
        Math.floor(Math.random() * byType[pickType].length),
        1
      )[0];
...
async function loadJSON(force = false) {
  try {
    ...
    if (!res.ok)
      throw new Error(
        "Failed to load physics.json — make sure it is served from same folder and accessible."
      );
    dataJSON = await res.json();
    metaRight.textContent = `Loaded: ${
      dataJSON.chapters ? dataJSON.chapters.length : "0"
    } chapters`;
    std.textContent = `${
      dataJSON.standerd ? dataJSON.standerd : "No standard found"
    }`;
    // create pool & start
    questionPool = buildQuestionPool(dataJSON);
    ...
    questionText.textContent =
      "Error loading physics.json — open console for details.";
    console.error(err);
